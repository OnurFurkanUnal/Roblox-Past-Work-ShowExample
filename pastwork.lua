...
local success, errorMessage = pcall(function()
	...
	for rank, data in ipairs(winsPage) do
		...
		if successs == true then
			...
			-- Show players on leaderboard object
...
game.Players.PlayerAdded:Connect(function(player)
	...
	for i, folder in game.ServerStorage.PlayerData:GetChildren() do
		local subData = data[folder.Name] or {} -- subdata datanın ıcınde dosya adıyla yoksa boş table olarak ata dedik burada
	...
	--add point to user data when player join the game
	...
	--Create inventorytemplate on player
...
--Check itemValues changed if 0 set visible false else visible true
...
-- assign events to inventory items buttons to drop them
...
-- when dropitem fired here will work
...
--show player to item name and that "f to pickup" text under true conditions
